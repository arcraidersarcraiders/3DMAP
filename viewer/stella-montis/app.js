...
document.addEventListener("pointerlockchange", () => {
  lockButton.textContent = document.pointerLockElement === canvas ? "鼠标已锁定" : "进入自由漫游";
});
...
async function loadManifest(href) {
  ...
  if (manifest.format === "stella-packed-scene-v1" && Array.isArray(manifest.chunks)) {
    ...
    scene.entries = manifest.chunks
      ...
      .filter(Boolean);
    if (scene.entries.length === 0) throw new Error("场景清单中没有任何数据块");
  } else {
    scene.format = "obj";
    scene.positionScale = [1, 1, 1];
    scene.preferSpatialStreaming = false;
    const meshes = normalizeManifest(manifest);
    if (meshes.length === 0) throw new Error("场景清单中没有任何网格");
  ...
  if (!hasFiniteBounds(scene.bounds)) throw new Error("场景清单中没有可用的网格坐标");
  setStatus(makeStats());
}

async function loadManifestIndex(manifestUrl) {
  const manifest = await fetchManifestVariant(manifestUrl);
  ...
  for (let index = 0; index < manifest.chunkParts.length; index += 1) {
    setStatus(`正在读取场景清单 ${index + 1}/${manifest.chunkParts.length}…`);
    ...
    if (Array.isArray(part)) {
      ...
      throw new Error(`场景分片中没有数据块：${partUrl.pathname}`);
    }
  }
  return {
    ...manifest,
...
// 优先读取压缩版清单（约 5 MB），找不到再回退到未压缩的 manifest.json。
// 这样同一份代码既能部署到静态托管（只上传 .json.gz），也能在本地保留明文清单时正常工作。
async function fetchManifestVariant(manifestUrl) {
  const candidates = manifestCandidates(manifestUrl);
  let lastError = null;
...
function manifestCandidates(manifestUrl) {
  if (!/manifest\.json$/i.test(manifestUrl.href)) return [manifestUrl];
  return [
    new URL(manifestUrl.href.replace(/manifest\.json$/i, "manifest.json.gz")),
    manifestUrl,
  ];
}
...
function parsePackedChunk(buffer, bounds) {
  if (buffer.byteLength < 16) throw new Error("数据块体积过小，可能已损坏");
  const magic = Array.from(new Uint8Array(buffer, 0, 8), (value) => String.fromCharCode(value)).join("");
  if (magic !== "STLBIN1\0") throw new Error("数据块头部标识未知，可能已损坏");
...
async function loadEntry(entry) {
  ...
  try {
    ...
    if (parsed.positions.length === 0 || drawCount === 0) throw new Error("网格为空");
    if (!entry.visible) {
      entry.state = "pending";
      return;
    }
...
async function fetchPackedArrayBuffer(entry) {
  ...
  if (compression === "gzip" && !hasPackedMagic(buffer)) {
    if (typeof DecompressionStream !== "function") {
      throw new Error("解压 gzip 数据块需要浏览器支持 DecompressionStream");
    }
...
function updateTelemetry(now) {
  ...
  telemetryEl.textContent = [
    `坐标 ${formatCoord(telemetry.position[0])}, ${formatCoord(telemetry.position[1])}, ${formatCoord(telemetry.position[2])}`,
    `朝向 ${formatDegrees(telemetry.heading)} 俯仰 ${formatSignedDegrees(telemetry.pitch)}`,
    `准星 ${formatProbeLine(aimedObjects[0] || aimedChunks[0])}`,
    `最近 ${formatProbeLine(nearestObjects[0] || nearestChunks[0])}`,
  ].join("\n");
...
async function copyTelemetrySnapshot(now) {
  ...
  if (!navigator.clipboard?.writeText) {
    setStatus(`${makeStats()} 探测信息已记录。`);
    return;
  }

  try {
    await navigator.clipboard.writeText(json);
    setStatus(`${makeStats()} 探测信息已复制。`);
  } catch (error) {
    console.warn("Could not copy stella probe", error, snapshot);
    setStatus(`${makeStats()} 探测信息已记录；剪贴板不可用。`);
  }
}
...
function primaryProbeName(chunk) {
  return chunk.name || chunk.sourceMeshes?.[0]?.name || chunk.sourcePackages?.[0]?.name || chunk.file || "未知";
}

function shortProbeName(value) {
  const base = String(value || "未知").split(/[\\/]/).pop() || "未知";
...
function makeStats() {
  ...
  const loading = scene.loadingCount ? `，${scene.loadingCount} 个加载中` : "";
  const failed = scene.failedCount ? `，${scene.failedCount} 个失败` : "";
  const budget = Number.isFinite(scene.residentBudget) ? `，上限 ${scene.residentBudget.toLocaleString()}` : "";
  const hidden = scene.hiddenBackdropCount ? `，${scene.hiddenBackdropCount.toLocaleString()} 个背景已隐藏` : "";
  const pending = scene.pendingCandidateCount ? `，${scene.pendingCandidateCount.toLocaleString()} 个待加载` : "";
  const speed = `速度 ${formatSpeed(camera.effectiveSpeed)}`;
  const scale = `缩放 ${formatScale(settings.scale)}x`;
  const xray = settings.xray ? "，透视" : "";
  return `常驻 ${resident}/${total}，已绘制 ${drawn}${budget}${hidden}${pending}；已加载 ${loaded}，三角形 ${drawnTriangles}/${triangles}，顶点 ${vertices}，${speed}，${scale}${xray}${loading}${failed}。`;
}
...
async function fetchJson(url) {
  ...
  let text;
  try {
    text = await decodeJsonBytes(buffer);
  } catch (error) {
    throw new Error(`${url.href}：内容解码失败（${error.message}）`);
  }
  if (!response.ok) {
  ...
  try {
    ...
    throw new Error(`${url.href}：JSON 解析失败（${error.message}）；${snippet(text)}`);
  }
}

// 清单可能以 manifest.json.gz 部署。各静态服务器对 .gz 的处理并不统一：
// 有的原样返回 gzip 字节，有的自动解压后返回明文，MIME 类型也五花八门。
// 因此一律先按内容判断是否为 gzip，再决定是否用 DecompressionStream 解压。
async function decodeJsonBytes(buffer) {
  if (!isGzipBytes(buffer)) return new TextDecoder("utf-8").decode(buffer);
  if (typeof DecompressionStream !== "function") {
    throw new Error("当前浏览器不支持 DecompressionStream，无法读取压缩清单");
  }
  const stream = new Blob([buffer]).stream().pipeThrough(new DecompressionStream("gzip"));
  return new Response(stream).text();
}

function isGzipBytes(buffer) {
  if (buffer.byteLength < 3) return false;
  const head = new Uint8Array(buffer, 0, 3);
  return head[0] === 0x1f && head[1] === 0x8b && head[2] === 0x08;
}
